...
import osdu.client.model.{StorageAcl, StorageLegal, StorageRecord};

class OSDUDataWriter(oakApiEndpoint: String,
                     partitionId: String,
                     bearerToken: String,
                     schema: StructType)
  ...
  def write(record: InternalRow): Unit = {
    ...
    storageRecord.setAcl(
      new StorageAcl()
        .owners( acl.getArray(idxAclOwners) .array.asInstanceOf[Array[String]].toSeq.asJava)
        .viewers(acl.getArray(idxAclViewers).array.asInstanceOf[Array[String]].toSeq.asJava))
    ...
    storageRecord.setLegal(
      new StorageLegal()
        .legaltags(                 legal.getArray(idxLegalTags) .array.asInstanceOf[Array[String]].toSeq.asJava)
        .otherRelevantDataCountries(legal.getArray(idxLegalOther).array.asInstanceOf[Array[String]].toSeq.asJava)
    ...
    // up to 500
    val createOrUpdateRecord = storageApi.createOrUpdateRecords("<unused>", true /* skipdups */, Seq(storageRecord).asJava)
  }
